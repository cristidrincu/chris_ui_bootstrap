$(document).ready(function(){
	...
	/* ---------- Make parent link clickable on double click  ---------- */
	
	/*$(".megamenu_drop").dblclick(function () {
	...
	/* ---------- Add asterisks to form labels to mark "required" ---------- */
	$('.required').append('<span class="asterisk">*</span>');
	
	/* ---------- Tooltip Controls ---------- */
	$('.clock-dropdown').data('tooltip').options.placement = 'left';
	
	/* ---------- Touchpunch implementation to make events work on touch devices ---------- */
	/*$('.sortable').draggable();
	$('.ui-sortable').draggable();
	
	$('.text-input').bind('click', function(){
	    $(this).focus();
	});*/
...
$.extend( $.fn.dataTableExt.oPagination, {
	"bootstrap": {
		"fnInit": function( oSettings, nPaging, fnDraw ) {
			...
			$(nPaging).addClass('pagination pagination-small pagination-centered').append(
				'<ul>'+
...
/* --------------- Menu Carousel ------------------------*/
	...
	$('#front_button').click(function(){
		...
		if(offset.left * -1 + windowWidth * 2 < $('.megamenu').width())
		{
			$('.megamenu').animate({
				right: '+='+windowWidth
			},1000);
		...
		{
			$('.megamenu').animate({
				right: $('.megamenu').width() - windowWidth
			},1000);
			$('#front_button').fadeOut();
		}
	});
	
	function showArrows()
	{
		if($('.megamenu').width() > $(window).width())
		{
			$('#front_button').fadeIn();
		}
		else
	...
	$(window).resize(function() {
		...
		resizeTimer = setTimeout(showArrows, 100);
	});
	
	/* --------------- Menu Carousel EOF------------------------*/
	
	/* --------------- Source View ------------------------*/
		$('.btn-source').dblclick(function(e){
		e.preventDefault();
		var source = $(this).removeClass('.btn-source').html();
		source = $.trim(vkbeautify.xml(source));	
		$('#sourceFormText').html(source);	
		$('#sourceCode').modal({
        backdrop: true,
        keyboard: true
		}).css({
	...
	$('#sourceCopy').click(function(){
		$('#sourceFormText').select();
		$('#copy-notification').slideDown();
	});
	
	$('#copy-notification .close').click(function() {
	   $('.alert').slideUp();
	});
